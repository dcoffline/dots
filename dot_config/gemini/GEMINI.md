...
- **Configuration Directories**: Always use XDG standard paths. Store all Gemini global personal memory files and rules in `~/.config/gemini` rather than `~/.gemini`.
...
## Behavioral Mandates
- **Clean Home Policy:** Gemini CLI configuration and memory are stored in `~/.config/gemini`. The `GEMINI_CLI_HOME` environment variable should be set to `~/.config/gemini`.
